...
- PPT Quality Gate structured findings schema and import
- PPT Deck Pro Max generation handoff dry-run import，并映射到现有 generation session 状态机
...
- suite readiness banner / Review Cockpit 状态集成，使用 `scripts/preview/server.py` 与 `scripts/preview/static/*`
...
B2 Quality Gate structured import
B3 Deck Pro Max handoff import
B4 Stack B QA
...
- PPT Quality Gate findings import 能映射到 Deck Master external quality gate classes。
- PPT Deck Pro Max generation handoff 复用现有 generation session 状态机；result import 后才影响 run state。
- feedback 默认只写 event queue，不默认写外部 PPT Library DB。
- 所有 external result 都进入 `imports/import_log.jsonl`。
- Review Cockpit 显示 suite readiness 和 blocking findings。
