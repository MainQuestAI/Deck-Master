...
- 云南白药真实 run 出现 `needs_page_review` 与外部生产放行并存的状态分裂。
...
```json
{
  ...
  "install_root": "/Users/<user>/.deck-master/current",
  "active_workspace": "",
  "default_runs_dir": "/Users/<user>/.deck-master/runs",
...
- 云南白药旧 run 能通过 `bind-workspace` 迁移。
...
- 云南白药 run 绑定 workspace 后，`request.json.workspace` 非空。
...
- `production_narrative` + 云南白药样本 -> 不出现库存、配送类页型。
...
以云南白药工作坊为 smoke case：

```bash
deck-master setup-status --workspace /Users/dingcheng/Workspace/云南白药/云南白药DAM
deck-master setup --workspace /Users/dingcheng/Workspace/云南白药/云南白药DAM --repair-workspace --target codex
deck-master bind-workspace --run-dir <yunnan-run> --workspace /Users/dingcheng/Workspace/云南白药/云南白药DAM --reason "Migrate pre-v0.9.11 run into workspace-bound chain."
