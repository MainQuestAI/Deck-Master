...
3. Keep raw source material outside the repository and point
   `source_material.local_source_paths` plus `inputs.context_pack` to approved
   local private paths.
...
reports should reference artifact paths and summarize status only. Do not add
raw customer text, source excerpts, caches, generated runs, or generated reports
to this folder.
